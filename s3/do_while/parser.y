...
%token START END READ WRITE PLUS MINUS MUL DIV ASSIGN NUM ID IF THEN ELSE ENDIF WHILE DO ENDWHILE BREAK CONTINUE LT GT EQ NEQ LE GE REPEAT UNTIL ENDREPEAT DOWHILE ENDDOWHILE
%left PLUS MINUS
%left MUL DIV

%type <node> NUM ID START END READ WRITE PLUS MINUS MUL DIV ASSIGN IF THEN ELSE ENDIF WHILE DO ENDWHILE BREAK CONTINUE LT GT EQ NEQ LE GE REPEAT DOWHILE UNTIL ENDDOWHILE ENDREPEAT
%type <node> program Slist Stmt InputStmt OutputStmt AsgStmt expr BreakStmt ContStmt IfStmt WhileStmt RepeatStmt DowhileStmt
...
DowhileStmt : DO Slist WHILE '(' expr ')' ENDDOWHILE';' { typecheck($5->type,BOOL,'d'),$$=createNode(VOID,0,NODE_DOWHILE,NULL,$5,$2,NULL);}
            ;
