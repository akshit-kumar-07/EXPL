void typecheck(int t1, int t2, char c) {
    switch(c) {
        ...
        case '=': if(t1 != t2) {
                  ...
                  break;
        case '&': if(t1 != TYPE_BOOL || t2 != TYPE_BOOL) {
                      yyerror("Invalid operands for AND operation\n");
                      exit(1);
                  }
                  break;    
        case '|': if(t1 != TYPE_BOOL || t2 != TYPE_BOOL) {
                      yyerror("Invalid operands for OR operation\n");
                      exit(1);
                  }
                  break;
        case 'r': if(t1 != TYPE_INT) {
        			yyerror("Expected integer expression inside read statement");
        			exit(1);
        		}
        		break;
