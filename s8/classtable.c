extern struct Paramstruct *argList1, *argList2;
...
void Class_Minstall(struct Classtable *cptr, char *name, struct Typetable *type, struct Paramstruct *paramlist) {
    ...
    if(mtemp != NULL) //in case the method is already defined
    {
        //Define the overriding case
        //Signature of the overriding function must be same as the original function

        //Check the return type
        if(mtemp->type != type)
        {
            yyerror("Return type for the overriding must be the same as the original function\n");
            exit(1);
        }

        //Check for the arguments types and names
        argList1 = paramlist; //the overriding function's paramlist
        argList2 = mtemp->paramlist; //original paramlist
...
struct Memberfunclist* copyClassMethods(struct Classtable *Ctemp)
{
    struct Memberfunclist *mtemp, *mIter = Ctemp->Vfuncptr;

    //Define a linked list to cpy the prev methods into
...
struct Classtable* CInstall(char *name, char *parent_class_name) {
    ...
    if(parent_class_name != NULL) 
    {
        ...
        //Check for existence of parent class
        ...
        //If the parent class exists, the child class will inherit all the fields and methods from this class

        Ctemp->memberfield = copyClassFields(Ctemp->parentptr);
        Ctemp->Vfuncptr = copyClassMethods(Ctemp->parentptr);
