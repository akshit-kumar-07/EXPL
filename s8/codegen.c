...
int nodeNewFuncPtrReg = -1; //holds the address where the VFT must be set for the next class
...
int codegen(struct ASTNode* t) {
    int r1, r2, r3, l1, l2,i, number, status=0;
    ...
    switch(t->nodetype) {
        ...
        case NODE_FIELDFUNC:
            ...
            //First push Self. 
            //Push the memory address of self
            r1 = codegen(t->ptr1);
            fprintf(intermediate, "PUSH R%d\n", r1);
            freeReg();

            //Push the Virtual Function table pointer
            r1 = getMemoryAddress(t->ptr1);
            fprintf(intermediate, "ADD R%d, 1\n", r1);
            fprintf(intermediate, "MOV R%d, [R%d]\n", r1, r1);
            fprintf(intermediate, "PUSH R%d\n", r1); //Argument 2
